...
// condition (if NodePoolsReady condition is set with status False).
func IsNodePoolsReadyFalse(cluster *capi.Cluster) bool {
	return capiconditions.IsFalse(cluster, NodePoolsReady)
}
...
// ReconcileNodePoolsReady sets NodePoolsReady condition on specified cluster
...
func ReconcileNodePoolsReady(cluster *capi.Cluster, nodePools []capiconditions.Getter) {
	if len(nodePools) == 0 {
